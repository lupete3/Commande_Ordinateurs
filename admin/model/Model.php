<?php 
	...
	class Model {
	    ...
	    public function produitExists($designation,$prix,$categorie){
	    	...
	    	$query = "SELECT * FROM produit WHERE designation = ? AND prix = ? AND id_cat = ?";

	      	$sql = $this->conn->prepare($query);

	      	$req = $sql->execute(array($designation,$prix,$categorie));
	    ...
	    //Méthode pour modifier un article dans ala base de données
	    public function editProduct($designation,$prix,$qte,$categorie,$disponible,$detail,$newname,$id){

	    	if (!empty($newname)) {
	    		$query = "UPDATE produit SET designation = ?,prix = ?,quantite = ?,id_cat = ?,disponible = ?,description = ?,image = ? WHERE id = ?";
	    		$sql = $this->conn->prepare($query);

		        if ($sql->execute(array($designation,$prix,$qte,$categorie,$disponible,$detail,$newname,$id))) {          
		        	return 1;
	    		...
	    		$query = "UPDATE produit SET designation = ?,prix = ?,quantite = ?,id_cat = ?,disponible = ?,description = ? WHERE id = ?";
	    		$sql = $this->conn->prepare($query);

		        if ($sql->execute(array($designation,$prix,$qte,$categorie,$disponible,$detail,$id))) {          
		        	return 1;
	    ...
	    //Méthode pour supprimer un produit dans ala base de données
